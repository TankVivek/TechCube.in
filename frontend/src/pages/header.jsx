...
function Header() {
  ...
  return (
    <header
      className={`w-full fixed top-0 left-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-sm shadow-md py-3" : "bg-transparent py-5"}`}
    >
      <div className="max-w-6xl mx-auto px-4 xl:px-0 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-4 lg:space-x-8">
          {["Home", "Services", "Expertise", "AI Solutions", "Contact"].map(
            (item, index) => (
              <motion.a
                key={item}
      ...
      <div
        className={` md:hidden fixed inset-0 z-40 transition-all duration-300 ease-out ${mobileMenuOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0 pointer-events-none"}`}
      >
        <div
